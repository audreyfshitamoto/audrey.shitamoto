/* ============================================================
   Home.tsx — AFS Portfolio v3 (optimized)
...
const HALL_OF_FAME: HallOfFameProject[] = [
  {
    index: '01',
...
const IntroCanvas = () => {
  ...
  useEffect(() => {
    ...
    const draw = () => {
      ...
      gradient.addColorStop(0, `rgba(255,255,255,${0.04 + Math.sin(animationTick * 0.03) * 0.02})`);
...
export default function Home() {
  ...
  const handleEnter = useCallback(() => {
    ...
    window.setTimeout(() => setIntroVisible(false), INTRO_EXIT_MS);
  }, []);

  useEffect(() => {
    const timeoutId = window.setTimeout(handleEnter, INTRO_DELAY_MS);
  ...
  const handleTopNavClick = useCallback((event: MouseEvent<HTMLAnchorElement>) => {
  ...
  return (
    <div style={{ background: '#000000', minHeight: '100vh', position: 'relative' }}>
      ...
      <nav className="afs-nav" style={{ opacity: introVisible ? 0 : 1, transition: 'opacity 0.8s ease 0.3s' }}>
        <a href="#" className="afs-nav-logo" onClick={handleTopNavClick}>AFS</a>
      ...
      >
        ...
        >
          {[0.2, 0.5, 0.8].map((position, index) => (
            <div
              ...
              style={{
                ...
                top: `${position * 100}%`,
                height: '1px',
                background: `rgba(255,255,255,${0.04 + index * 0.02})`,
              }}
            />
          ))}
          ...
          {[['0', '0'], ['0', 'auto'], ['auto', '0'], ['auto', 'auto']].map(([top, bottom], index) => (
            <div
              key={index}
              style={{
                position: 'absolute',
                top: top === '0' ? '1.5rem' : 'auto',
                bottom: bottom === 'auto' ? '1.5rem' : 'auto',
                left: index < 2 ? '2.5rem' : 'auto',
                right: index >= 2 ? '2.5rem' : 'auto',
                width: '20px',
                height: '20px',
                borderTop: top === '0' ? '1px solid rgba(255,255,255,0.15)' : 'none',
                borderBottom: bottom === 'auto' ? '1px solid rgba(255,255,255,0.15)' : 'none',
                borderLeft: index < 2 ? '1px solid rgba(255,255,255,0.15)' : 'none',
        ...
        <section id="portfolio" ref={portfolioRef} className="portfolio-section" style={{ position: 'relative', overflow: 'hidden' }}>
          ...
          <div className="portfolio-grid" style={{ position: 'relative', zIndex: 1 }}>
            {PORTFOLIO_ITEMS.map((item, index) => (
              <div key={item.num} className={`portfolio-item reveal reveal-delay-${(index % 3) + 1}`}>
        ...
        <section className="education-section">
          ...
          <div className="education-entries">
            {EDUCATION.map((entry, index) => (
              <div key={entry.institution} className={`education-entry reveal reveal-delay-${index + 1}`}>
                <div>
          ...
          <div className="skills-grid">
            ...
            <div className="skills-content">
              {SKILLS.map((category, categoryIndex) => (
                <div
                  key={category.label}
                  className={`skill-category reveal reveal-delay-${categoryIndex + 1}`}
                  style={{ animationDelay: `${categoryIndex * 0.12}s` }}
                >
                  <div className="skill-category-label">{category.label}</div>
                  <div className="skill-tags">
                    {category.tags.map((tag, tagIndex) => (
                      <span
                        ...
                        className="skill-tag"
                        style={{
                          animationDelay: `${categoryIndex * 0.1 + tagIndex * 0.04}s`,
                          opacity: 0,
        ...
        <section className="certifications-section">
          ...
          <div className="certifications-grid">
            {CERTIFICATIONS.map((cert, index) => (
              <div key={cert.title} className={`cert-card reveal reveal-delay-${(index % 4) + 1}`}>
